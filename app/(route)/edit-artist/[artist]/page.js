...
const ArtistRegistration = ({ params }) => {
  ...
  const getArtist = async () => {
    ...
    try {
      ...
      setGalleryImages(artistData.gallery);
  ...
  const handleGallerySave = async () => {
    ...
    try {
      const croppedImg = await getCroppedImg(gallerySrc, croppedArea);
      setGalleryImages((prevImages) => [...prevImages, croppedImg]);
      await handleGalleryUpload(croppedImg);
  ...
  const deleteImage = async (imageUrl) => {
    try {
      // Parse the image URL to extract the bucket name and object key
      const objectKey = new URL(imageUrl).pathname.slice(1); // Remove the leading slash
      ...
      setProfilePic("");
      setShowCroppedImage(false); // Hide the cropped image
  ...
  const deleteGalleryImage = async (index, imageUrl) => {
    try {
      // Delete image from AWS S3
      // Implement logic to delete the image from S3 using its URL
      // Parse the image URL to extract the bucket name and object key
      const objectKey = new URL(imageUrl).pathname.slice(1); // Remove the leading slash
      console.log(objectKey);
      ...
      // Send delete object command to S3
      await s3Client.send(deleteCommand);
      // Remove the image from the gallery array in state
          ...
          {profilePic && (
            <div className="mb-4">
              ...
              {/* <Button
                  ...
                  onClick={() => deleteImage(profilePic)}
                >
                  Delete
                </Button> */}
            </div>
        ...
        gap-7 mt-4
         lg:grid-cols-4"
          >
            ...
            </Dropzone>
            {
              !galleryImages.map((imageUrl, index) => (
                <div key={index}>
                  ...
                  />
                  {/* <button
                  onClick={() => deleteGalleryImage(index, imageUrl)}
                  className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                >
                  Delete
                </button> */}
            ...
            {galleryImages && showGalleryModal && gallerySrc && (
              <div className="modal-overlay">
                <div className="modal flex flex-col gap-4">
                  <Cropper
                    src={gallerySrc}
              ...
              {eventTypesOptions.map((option) => (
                <div className="" key={option}>
                  <input
                    ...
                    className="mr-2"
                    // Check if the current event type is included in the selected event types
